...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  // throw new Error('Not implemented');
  const lineOne = Math.abs(x2 - x1) ** 2;
  const lineTwo = Math.abs(y2 - y1) ** 2;

  return Math.sqrt(lineOne + lineTwo);
}
...
function getCountOfOddNumbers(number) {
  ...
  return numberArr.filter((item) => item % 2 != 0).length;
}
